...
static ProcessNode* checkNewJobsIncoming (ProcessNode *lastAvailable, double timeElapsed, ProcessNode *queue, unsigned int *anyNewProcess) {
	while (lastAvailable->next != NULL && lastAvailable->next->val->t0 <= timeElapsed) {
		lastAvailable = lastAvailable->next;

		if (debugging) {
			fprintf (stderr, "(t = %lf) Incoming job: %s (%d) - t0: %lf | dt = %lf | r: %lf | q: %lf\n",
								timeElapsed,
								lastAvailable->val->name,
								lastAvailable->val->id,
								lastAvailable->val->t0,
								lastAvailable->val->dt,
								lastAvailable->val->remaining,
								lastAvailable->val->quantum);
		}

		insert_queue (queue, lastAvailable->val);
...
void start_multiple_queues (char *input_file, char *output_file, int debug) {
	debugging = debug;
	...
	ProcessNode *queue = create_queue ();
	...
	while (jobsFinished < jobs) {
		...
		anyNewProcess = 0;
		lastAvailable = checkNewJobsIncoming (lastAvailable, timeElapsed, queue, &anyNewProcess);
		...
		if (currentJob != NULL && timeElapsedIteration >= currentJob->remaining) {
			...
			*/
			...
			/* Print if debugging */
			if (debugging) {
				fprintf (stderr, "Job finished: %s (%d) - t0: %lf | dt = %lf | r: %lf | q: %lf\n", currentJob->name, currentJob->id, currentJob->t0, currentJob->dt, currentJob->remaining, currentJob->quantum);
			}

			currentJob = previousJob = NULL;

		} else if (currentJob != NULL && timeElapsedIteration >= currentJob->quantum) {
			/* Save current state */
			currentJob->remaining -= timeElapsedIteration;
			currentJob->quantum *= 2;
			gettimeofday(&lastTime, NULL);

			/* Enqueue current job */
			insert_queue (queue, currentJob);

			/* Save previous job */
			previousJob = currentJob;

			/* Set current to NULL (in order to find another) */
			currentJob = NULL;
		}

		/* If there is no current job, let's take one. */
		if (currentJob == NULL) {
			/* Try to take another job */
			currentJob = pop_queue (queue);
